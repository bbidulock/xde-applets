...
=head1 NAME
 ...
 xde-id-applet - an X11 input device system tray applet

=head1 SYNOPSIS

B< xde-id-applet> [ I<OPTIONS> ]
 B<xde-id-applet> --quit [ I<OPTIONS> ]
 B<xde-id-applet> --replace [ I<OPTIONS> ]
 B<xde-id-applet> --help [ I<OPTIONS> ]
 B<xde-id-applet> --version
 B<xde-id-applet> --copying
...
running session of B<xde-id-applet>.
...
B<xde-id-applet> plays the following sounds under the following
...
B<xde-id-applet> examines the following environment variables:
...
I wrote B<xde-id-applet> due to the deficiencies and lack of
