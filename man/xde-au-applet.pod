...
=head1 NAME
 ...
 xde-au-applet - an Avahi system tray applet

=head1 SYNOPSIS

B< xde-au-applet> [ I<OPTIONS> ]
 B<xde-au-applet> --quit [ I<OPTIONS> ]
 B<xde-au-applet> --replace [ I<OPTIONS> ]
 B<xde-au-applet> --help [ I<OPTIONS> ]
 B<xde-au-applet> --version
 B<xde-au-applet> --copying
...
running session of B<xde-au-applet>.
...
B<xde-au-applet> plays the following sounds under the following
...
B<xde-au-applet> examines the following environment variables:
...
I wrote B<xde-au-applet> due to the deficiencies and lack of
availability of system tray and window-maker dock application
audio management and monitoring.
The program was originally written in C<perl,> but later ported to C<C>.

The L<pavucontrol(1)> applet is an excellent applet; however, it is a bit overkill and does not
provide very usable tool-tip pop-ups.  Although it can control all aspects of pulse audio,
something as simple a muting the microphone somehow escapes it.  It also
lacks hot-key and special function key capabilities.
