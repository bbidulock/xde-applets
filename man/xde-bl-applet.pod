...
=head1 SYNOPSIS
 ...
 xde-bl-applet [ OPTIONS ]
 xde-bl-applet --quit [ OPTIONS ]
 xde-bl-applet --replace [ OPTIONS ]
 xde-bl-applet --help [ OPTIONS ]
 xde-bl-applet --version
 xde-bl-applet --copying
...
display, and the color tint (blue light filtration) of the display.  THe
state of all back-lights and LEDs are displayed in a tool-tip pop-up and
the overall state of the lights is shown in a status icon.  It also can
be used to play sound theme specific sounds associated with changes to
the display.
...
availability of system tray and window-maker dock application backlight
controls.  The program was originally written in C<perl,> but later
ported to C<C>.
