...
#define RESNAME "xde-lm-applet"
#define RESCLAS "XDE-LM-Applet"
#define RESTITL "XDE LM Sensors Applet"
#define RESCOMM "An LM sensors system tray icon and dock application."
...
update_sensors(XdeScreen *xscr)
{
	...
	for (chip = xscr->chips; chip; chip = chip->next) {
		...
		for (feat = xchip->features; feat; feat = feat->next) {
			...
			XdeSubfeature *xsubf;
			char *markup;
			double input, min, max, low, cri, perc;
	...
	if (temp_range && temp_range != old_temp) {
		...
		case TempFlameCold:
			...
			break;
		case TempFlameCool:
			id = (old_temp > TempFlameWarm) ? "thermal-cool" : NULL;
			break;
		case TempFlameWarm:
			id = (old_temp > TempFlameWarm) ? "thermal-cool" : NULL;
			break;
		case TempFlameHot:
			id = (old_temp < temp_range) ? "thermal-caution" : NULL;
			break;
		case TempFlameBurn:
			id = "thermal-hot";
			break;
