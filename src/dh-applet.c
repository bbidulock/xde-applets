...
#define XA_SELECTION_NAME	"_DH_APPLET_S%d"

#define LOGO_NAME		"dhcpcd"
...
Atom _XA_DH_APPLET_REFRESH;
Atom _XA_DH_APPLET_RESTART;
Atom _XA_DH_APPLET_POPMENU;
Atom _XA_DH_APPLET_REQUEST;
...
on_about_selected(GtkMenuItem *item, gpointer user_data)
{
	...
	gtk_show_about_dialog(NULL,
			      "authors", authors,
			      "comments", "A DHCP client daemon system tray icon.",
...
event_handler_ClientMessage(Display *dpy, XEvent *xev)
{
	...
	} else if (xscr && xev->xclient.message_type == _XA_DH_APPLET_REFRESH) {
		// set_flags(xev->xclient.data.l[2]);
		// set_word1(xev->xclient.data.l[3]);
		// set_word2(xev->xclient.data.l[4]);
		applet_refresh(xscr);
		return GDK_FILTER_REMOVE;
	} else if (xscr && xev->xclient.message_type == _XA_DH_APPLET_RESTART) {
		// set_flags(xev->xclient.data.l[2]);
		// set_word1(xev->xclient.data.l[3]);
		// set_word2(xev->xclient.data.l[4]);
		applet_restart();
		return GDK_FILTER_REMOVE;
	} else if (xscr && xev->xclient.message_type == _XA_DH_APPLET_POPMENU) {
		// set_flags(xev->xclient.data.l[2]);
		// set_word1(xev->xclient.data.l[3]);
		// set_word2(xev->xclient.data.l[4]);
		// menu_popmenu(xscr);
...
startup(int argc, char *argv[], Command command)
{
	...
	atom = gdk_atom_intern_static_string("_DH_APPLET_REFRESH");
	_XA_DH_APPLET_REFRESH = gdk_x11_atom_to_xatom_for_display(disp, atom);

	atom = gdk_atom_intern_static_string("_DH_APPLET_RESTART");
	_XA_DH_APPLET_RESTART = gdk_x11_atom_to_xatom_for_display(disp, atom);

	atom = gdk_atom_intern_static_string("_DH_APPLET_POPMENU");
	_XA_DH_APPLET_POPMENU = gdk_x11_atom_to_xatom_for_display(disp, atom);

	atom = gdk_atom_intern_static_string("_DH_APPLET_REQUEST");
	_XA_DH_APPLET_REQUEST = gdk_x11_atom_to_xatom_for_display(disp, atom);
...
do_refresh(int argc, char *argv[])
{
	...
	for (s = 0; s < nscr; s++) {
		...
		if ((owner = XGetSelectionOwner(dpy, atom)) && gotone != owner) {
			...
			ev.xclient.display = dpy;
			ev.xclient.window = RootWindow(dpy, s);
			ev.xclient.message_type = _XA_DH_APPLET_REFRESH;
			ev.xclient.format = 32;
			ev.xclient.data.l[0] = CurrentTime;
...
do_restart(int argc, char *argv[])
{
	...
	for (s = 0; s < nscr; s++) {
		...
		if ((owner = XGetSelectionOwner(dpy, atom)) && gotone != owner) {
			...
			ev.xclient.display = dpy;
			ev.xclient.window = RootWindow(dpy, s);
			ev.xclient.message_type = _XA_DH_APPLET_RESTART;
			ev.xclient.format = 32;
			ev.xclient.data.l[0] = CurrentTime;
			ev.xclient.data.l[1] = atom;
