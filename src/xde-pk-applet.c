...
#define XA_PREFIX		"_XDE_PK_APPLET"
#define XA_SELECTION_NAME	XA_PREFIX "_S%d"
#define LOGO_NAME		"system-software-install"

int saveArgc;
char **saveArgv;

#define RESNAME "xde-pk-applet"
#define RESCLAS "XDE-PK-Applet"
#define RESTITL "XDE Package Kit Applet"
...
Atom _XA_XDE_PK_APPLET_REFRESH;
Atom _XA_XDE_PK_APPLET_RESTART;
Atom _XA_XDE_PK_APPLET_POPMENU;
Atom _XA_XDE_PK_APPLET_REQUEST;
...
on_about_selected(GtkMenuItem *item, gpointer user_data)
{
	...
	gtk_show_about_dialog(NULL,
			      "authors", authors,
			      "comments", "A software update system tray icon.",
...
on_popup_menu(GtkStatusIcon *icon, guint button, guint time, gpointer user_data)
{
	...
	item = gtk_image_menu_item_new_with_label("Packages...");
	imag = gtk_image_new_from_icon_name("system-software-install", GTK_ICON_SIZE_MENU);
	gtk_image_menu_item_set_image(GTK_IMAGE_MENU_ITEM(item), imag);
	g_signal_connect(item, "activate", G_CALLBACK(on_packages_selected), xscr);
	gtk_widget_show(item);
	gtk_menu_append(menu, item);

	item = gtk_image_menu_item_new_with_label("Updates...");
	imag = gtk_image_new_from_icon_name("system-software-update", GTK_ICON_SIZE_MENU);
	gtk_image_menu_item_set_image(GTK_IMAGE_MENU_ITEM(item), imag);
	g_signal_connect(item, "activate", G_CALLBACK(on_updates_selected), xscr);
	gtk_widget_show(item);
	gtk_menu_append(menu, item);

	item = gtk_image_menu_item_new_with_label("Repos...");
	imag = gtk_image_new_from_icon_name("system-file-manager", GTK_ICON_SIZE_MENU);
	gtk_image_menu_item_set_image(GTK_IMAGE_MENU_ITEM(item), imag);
	g_signal_connect(item, "activate", G_CALLBACK(on_repos_selected), xscr);
	gtk_widget_show(item);
	gtk_menu_append(menu, item);
...
event_handler_ClientMessage(Display *dpy, XEvent *xev)
{
	...
	} else if (xscr && xev->xclient.message_type == _XA_XDE_PK_APPLET_REFRESH) {
		// set_flags(xev->xclient.data.l[2]);
		// set_word1(xev->xclient.data.l[3]);
		// set_word2(xev->xclient.data.l[4]);
		applet_refresh(xscr);
		return GDK_FILTER_REMOVE;
	} else if (xscr && xev->xclient.message_type == _XA_XDE_PK_APPLET_RESTART) {
		// set_flags(xev->xclient.data.l[2]);
		// set_word1(xev->xclient.data.l[3]);
		// set_word2(xev->xclient.data.l[4]);
		applet_restart();
		return GDK_FILTER_REMOVE;
	} else if (xscr && xev->xclient.message_type == _XA_XDE_PK_APPLET_POPMENU) {
		// set_flags(xev->xclient.data.l[2]);
		// set_word1(xev->xclient.data.l[3]);
		// set_word2(xev->xclient.data.l[4]);
		// menu_popmenu(xscr);
...
startup(int argc, char *argv[], Command command)
{
	...
	atom = gdk_atom_intern_static_string("_XDE_PK_APPLET_REFRESH");
	_XA_XDE_PK_APPLET_REFRESH = gdk_x11_atom_to_xatom_for_display(disp, atom);

	atom = gdk_atom_intern_static_string("_XDE_PK_APPLET_RESTART");
	_XA_XDE_PK_APPLET_RESTART = gdk_x11_atom_to_xatom_for_display(disp, atom);

	atom = gdk_atom_intern_static_string("_XDE_PK_APPLET_POPMENU");
	_XA_XDE_PK_APPLET_POPMENU = gdk_x11_atom_to_xatom_for_display(disp, atom);

	atom = gdk_atom_intern_static_string("_XDE_PK_APPLET_REQUEST");
	_XA_XDE_PK_APPLET_REQUEST = gdk_x11_atom_to_xatom_for_display(disp, atom);
...
do_refresh(int argc, char *argv[])
{
	...
	for (s = 0; s < nscr; s++) {
		...
		if ((owner = XGetSelectionOwner(dpy, atom)) && gotone != owner) {
			...
			ev.xclient.display = dpy;
			ev.xclient.window = RootWindow(dpy, s);
			ev.xclient.message_type = _XA_XDE_PK_APPLET_REFRESH;
			ev.xclient.format = 32;
			ev.xclient.data.l[0] = CurrentTime;
...
do_restart(int argc, char *argv[])
{
	...
	for (s = 0; s < nscr; s++) {
		...
		if ((owner = XGetSelectionOwner(dpy, atom)) && gotone != owner) {
			...
			ev.xclient.display = dpy;
			ev.xclient.window = RootWindow(dpy, s);
			ev.xclient.message_type = _XA_XDE_PK_APPLET_RESTART;
			ev.xclient.format = 32;
			ev.xclient.data.l[0] = CurrentTime;
			ev.xclient.data.l[1] = atom;
