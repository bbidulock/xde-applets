...
#define XA_PREFIX		"_XDE_WL_APPLET"
...
#define RESNAME "xde-wl-applet"
#define RESCLAS "XDE-WL-Applet"
#define RESTITL "XDE Wireless Applet"
#define RESCOMM "A wireless system tray icon and dock application."
...
void
on_networks_selected(GtkMenuItem *item, gpointer user_data)
{
	(void) item;
	(void) user_data;
}
...
void
on_interfaces_selected(GtkMenuItem *item, gpointer user_data)
{
	(void) item;
	(void) user_data;
}
...
get_popup_menu(XdeScreen *xscr)
{
	...
	item = gtk_image_menu_item_new_with_label("Networks...");
	imag = gtk_image_new_from_icon_name("network", GTK_ICON_SIZE_MENU);
	gtk_image_menu_item_set_image(GTK_IMAGE_MENU_ITEM(item), imag);
	g_signal_connect(item, "activate", G_CALLBACK(on_networks_selected), xscr);
	gtk_widget_show(item);
	gtk_menu_append(menu, item);

	item = gtk_image_menu_item_new_with_label("Interfaces...");
	imag = gtk_image_new_from_icon_name("network-wireless", GTK_ICON_SIZE_MENU);
	gtk_image_menu_item_set_image(GTK_IMAGE_MENU_ITEM(item), imag);
	g_signal_connect(item, "activate", G_CALLBACK(on_interfaces_selected), xscr);
	gtk_widget_show(item);
	gtk_menu_append(menu, item);
